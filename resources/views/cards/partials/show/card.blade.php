                    ...
                    <div class="card-body">
                        <p class="card-text">
                            @if($card->lastMatch() && $card->lastDispense())
                                ...
                                @else
                                    <a href="https://xchain.io/tx/{{ $card->lastDispense()->dispenser_tx_hash }}" target="_blank">{{ number_format($card->lastDispense()->dispenser->trading_price_normalized, 8) }} BTC</a>
                                @endif
